...
int add_arp_cache_entry(unsigned char * device_name, unsigned char * dest, unsigned char dest_mac[6]) { // *dest="Dongdaemungu 1"(�̰� ���⼭ IP ����)
...
int add_routing_entry(unsigned char * dest, unsigned char * next_hop, unsigned char * output_interface) { //dest="Dongdaemungu"
...
/*
답:
ROUTING TABLE에서 DESTINATION IP와 longest matching되는 ENTRY를 찾기 위해 필요하다. 
패킷을 destination으로 보내기 위해 해당되는 entry를 찾아서, 그 다음에 해당 entry의 next hop과 interface를 보고 거기로 forwarding을 해야하기 때문에
이 함수의 작업이 우선적으로 필요하다.
*/
...
int my_ip_output(struct my_ip_header my_iph, struct my_ip_routing_entry * rentry) { //rentry: routing entry���� ã�� entry
	struct my_arp_cache_entry * cache_ptr; //ARP: IP�� �ش��ϴ� MAC �ּ�
...
int my_ip_forward(struct my_ip_header my_iph) { //rougting 테이블에서 entry 찾아서 ttl 감소시키기
...
void environment_setup() {
	unsigned char mac[6];
	sscanf("08:00:27:1f:5d:56", "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]);
