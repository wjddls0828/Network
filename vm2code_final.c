...
패킷을 destination으로 보내기 위해 해당되는 entry를 찾아서, 그 다음에 해당 entry의 next hop을 보고 거기로 forwarding을 해야하기 때문에
...
void environment_setup() {
	...
	sscanf("08:00:27:8f:e6:dc", "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]);
	add_arp_cache_entry("enp0s3", "Dongdaemungu 1", mac);
	sscanf("08:00:27:4c:d9:f6", "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]);
